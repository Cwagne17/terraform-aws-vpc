...
func TestTerraformAwsVpcComplete(t *testing.T) {
	t.Parallel()
	...
	// in your AWS account
	vpcName := fmt.Sprintf("complete-example-%s", strings.ToLower(random.UniqueId()))
	...
	assert.Equal(t, vpcName, vpc.Name, "VPC name does not match")

	// Check that a correct number of availability zones were used in the VPC
	availabilityZonesInRegion := aws.GetAvailabilityZones(t, awsRegion)
	assert.Equal(t, len(availabilityZones), len(availabilityZonesInRegion), "Number of availability zones used in the VPC does not match the number of availability zones in the region")
	...
	for i := range vpc.Subnets {
		if aws.IsPublicSubnet(t, vpc.Subnets[i].Id, awsRegion) {
			assert.Contains(t, publicSubnetIds, vpc.Subnets[i].Id, "Public subnet not found in public_subnet_ids")
			publicSubnetCount++
		} else {
	...
	assert.Equal(t, len(publicSubnetIds), publicSubnetCount, "Number of public subnets does not match")
	assert.Equal(t, len(privateSubnetIds), privateSubnetCount, "Number of private subnets does not match")

	// Check that the number of public and private subnets match the number of availability zones
	assert.Equal(t, len(availabilityZones), publicSubnetCount, "Number of public subnets does not match the number of availability zones")
	assert.Equal(t, len(availabilityZones), privateSubnetCount, "Number of private subnets does not match the number of availability zones")
}
