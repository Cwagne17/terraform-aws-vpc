...
func TestTerraformAwsVpcPublicSubnetsOnly(t *testing.T) {
	t.Parallel()

	// Make a copy of the terraform module to a temporary directory. This allows running multiple tests in parallel
	// against the same terraform module.
	exampleFolder := test_structure.CopyTerraformFolderToTemp(t, "../", "examples/public-only")

	// Give this VPC a unique ID for a name tag so we can distinguish it from any other VPC provisioned
	// in your AWS account
	vpcName := fmt.Sprintf("public-only-example-%s", strings.ToLower(random.UniqueId()))
	...
	assert.Equal(t, 0, privateSubnetCount, "Number of private subnets is not 0")
}
